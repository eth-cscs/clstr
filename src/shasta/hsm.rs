...
pub mod utils {
    ...
    pub fn get_list_node_hw_inventory_and_counter_filtered_by_pattern(
        node_hw_inventory_value: &Value,
        pattern_hw_inv: Vec<String>,
    ) -> Vec<(String, u8)> {
        let sol = vec![];
        let processor = get_list_processor_model_from_hw_inventory_value(&node_hw_inventory_value)
            .unwrap_or_default();

        let mut processor_map = HashMap::new();
        for mut value in processor {
            value.make_ascii_lowercase();
            let pattern_option = pattern_hw_inv
                .iter()
                .find(|&pattern| value.contains(pattern));
            if let Some(pattern) = pattern_option {
                *processor_map.entry(pattern.clone()).or_insert(0) += 1;
            }
        }

        println!("Count for processors: {:?}", processor_map);

        let accelerator =
            get_list_accelerator_model_from_hw_inventory_value(&node_hw_inventory_value)
                .unwrap_or_default();
        let mut accelerator_map = HashMap::new();
        for mut value in accelerator {
            value.make_ascii_lowercase();
            let pattern_option = pattern_hw_inv
                .iter()
                .find(|&pattern| value.contains(pattern));
            if let Some(pattern) = pattern_option {
                *accelerator_map.entry(pattern.clone()).or_insert(0) += 1;
            }
        }

        println!("Count for accelerators: {:?}", accelerator_map);
